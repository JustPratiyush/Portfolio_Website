...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const projectsContainer = document.getElementById("projects-container") || document.querySelector("#projects .row.g-4");
  if (projectsContainer || isProjectsPage) {
    const containerId = projectsContainer ? projectsContainer.id : 'projects-container';
    const container = containerId === 'projects-container' ? projectsContainer : document.getElementById(containerId);
...
async function loadContent(options) {
  const { containerId, dataPath, cardGenerator, limit = 0, isBlogsPage = false, isProjectsPage = false } = options;
  console.group(`Loading content for: ${containerId}`);
  console.log('Options:', { containerId, dataPath, limit, isBlogsPage, isProjectsPage });
  ...
  try {
    ...
    data.forEach((item, index) => {
      try {
        console.log(`Creating card for item ${index + 1}:`, item.title || 'Untitled');
        ...
        errorCard.innerHTML = `
          <div class="alert alert-warning">
            ...
            ${process.env.NODE_ENV === 'development' ? `<br><small>${cardError.message}</small>` : ''}
          </div>`;
      ...
      <a href="${url}" class="blog-card-link text-decoration-none text-reset d-block h-100">
        <div class="card h-100 shadow-sm border-0 overflow-hidden">
          ...
          <div class="card-body d-flex flex-column">
            <div class="mb-2">
              ${date || readTime ? `<p class="text-muted small mb-2">${[date, readTime].filter(Boolean).join(' · ')}</p>` : ''}
              <h4 class="card-title h5 mb-3">${post.title || 'Untitled Post'}</h4>
...
function createProjectCard(project) {
  ...
  const primaryUrl = (project.demoUrl && project.demoUrl !== "#") 
    ? project.demoUrl 
